...
❌ A flat listing, count, or filter of ONE resource type → `subimage-mcp:inventory-via-cypher`, which maps the type to its ontology label, or to the provider-native label when the user named a specific product, without schema discovery.
