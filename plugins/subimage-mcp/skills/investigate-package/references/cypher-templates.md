...
Use this only to compare scanner presence for the selected image. Both the canonical package and each scanner node are anchored to that image through their own `DEPLOYED` edge, so a detection from a different image cannot fill either array. Scanner agreement does not establish package origin.
...
OPTIONAL MATCH (p)-[trivyDetected:DETECTED_AS]->(trivy:TrivyPackage)-[trivyDeployed:DEPLOYED]->(i)
OPTIONAL MATCH (p)-[syftDetected:DETECTED_AS]->(syft:SyftPackage)-[syftDeployed:DEPLOYED]->(i)
...
If you do not yet have an image digest, drop the `DEPLOYED`/digest predicates to see scanner coverage across all images, but treat the result as package-level only: it cannot confirm both scanners saw the package on any single image.
...
Use this when a CVE is in scope. `TrivyImageFinding` is shared by vulnerability id, so independently matching its package and image relationships can combine a package from another image. Require the package to be deployed on the selected image, and accept the finding's layer only when `HAS_LAYER` or `Image.layer_diff_ids` verifies image membership.

```cypher
MATCH (f:TrivyImageFinding)-[affectsPackage:AFFECTS]->(p:Package)
MATCH (f)-[affectsImage:AFFECTS]->(i:Image)
MATCH (p)-[deployed:DEPLOYED]->(i)
...
RETURN p.purl AS purl,
       ...
       layer IS NOT NULL AS layer_attribution_valid,
