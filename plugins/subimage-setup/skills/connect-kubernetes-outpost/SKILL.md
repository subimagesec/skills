...
The Tailscale credential is the **Outpost registration key** in SubImage, not generated by the user. **If any input is missing, ask the user explicitly.**
...
| `<TAILSCALE_AUTHKEY>` | Outpost registration key from **Settings → Outposts**. Format: `tskey-client-...`.                                                       | "Can an admin copy the Outpost registration key from Settings → Outposts? It starts with `tskey-client-`. If the page or key is missing, contact SubImage support." |
...
- **All outposts for the same tenant share the same registration key and Tailscale tag.** Only the hostname (`<TENANT_ID>-<NAME>-outpost`) differs. Do not ask SubImage for separate keys per outpost; reuse the one you have.
